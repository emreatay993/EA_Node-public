...
GALLERY_QML_PATH = MOCKUP_ROOT / "NodeCommentsGallery.qml"
VARIANT_IDS = ("1",)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Launch node comment badge and inspector QML mockups.",
    )
    parser.add_argument(
        "--variant",
...
def _create_view(*, variant: str, theme: str, screenshot_mode: bool) -> QQuickView:
    view = QQuickView()
    view.setTitle("COREX Node Comments Mockup")
    view.setResizeMode(QQuickView.ResizeMode.SizeRootObjectToView)
    view.setColor(QColor("#1d1f24"))
    view.setWidth(1320 if screenshot_mode else 1480)
    view.setHeight(820 if screenshot_mode else 930)
    _register_mockup_context(
        view,
        variant=variant,
        theme=theme,
...
def _capture_screenshots(app: QApplication, args: argparse.Namespace) -> int:
    ...
    for variant in _requested_variants(args.variant):
        for theme in _requested_themes(args.theme):
            ...
            output_path = screenshot_dir / f"node_comments_variant_{variant}_{theme}.png"
