...
GALLERY_QML_PATH = MOCKUP_ROOT / "StatusBarGallery.qml"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Launch status / performance bar QML mockups (5 design routes).",
    )
    parser.add_argument(
        "--variant",
...
def _create_view(*, variant: str, theme: str, screenshot_mode: bool) -> QQuickView:
    view = QQuickView()
    view.setTitle("COREX Status / Performance Bar Mockups")
    view.setResizeMode(QQuickView.ResizeMode.SizeRootObjectToView)
    view.setColor(QColor("#1d1f24"))
    view.setWidth(1180 if screenshot_mode else 1480)
    view.setHeight(760 if screenshot_mode else 980)
...
def _settle(ms: int) -> None:
    """Advance real wall-clock so startup animations finish before capture."""
    loop = QEventLoop()
...
def _capture_screenshots(app: QApplication, args: argparse.Namespace) -> int:
    ...
    for variant in _requested_variants(args.variant):
        for theme in _requested_themes(args.theme):
            ...
            view.show()
            _pump_events(app, 8)
            _settle(480)  # let mini-bar / sweep / pulse animations finish
            _pump_events(app, 4)
            image = view.grabWindow()
            output_path = screenshot_dir / f"status_bar_variant_{variant}_{theme}.png"
