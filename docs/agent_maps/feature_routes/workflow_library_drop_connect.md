...
- Forwarded anchors resolve through `GraphTypeResolver`; mixed outputs must satisfy the destination for every source member. Recommendations retain the actual dragged node/port as the insertion endpoint, including after Panel or Trigger. The controller revalidates current topology and restricted selected ports after creation; inferred types never replace the anchor. `tests/test_type_forwarding_ui.py` owns presenter/controller insertion, stale choices, and real QML coverage.
