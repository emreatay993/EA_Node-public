...
from ea_node_editor.mockups.node_comments.__main__ import GALLERY_QML_PATH, MOCKUP_ROOT
...
VARIANT_PATHS = [
    MOCKUP_ROOT / "variants" / "NodeCommentUnifiedVariant.qml",
]
...
def test_node_comment_gallery_and_variant_compile(qapp) -> None:  # noqa: ANN001
    engine = _new_engine()
...
def test_node_comment_runner_smoke() -> None:
    env = os.environ.copy()
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "ea_node_editor.mockups.node_comments",
            "--quit-after-ms",
            "500",
        ],
        cwd=REPO_ROOT,
...
def test_node_comment_runner_writes_theme_screenshots(tmp_path: Path) -> None:
    env = os.environ.copy()
    env["QT_QPA_PLATFORM"] = "offscreen"
    env.setdefault("QT_QUICK_CONTROLS_STYLE", "Basic")
    screenshot_dir = tmp_path / "node_comments"
    ...
    assert result.returncode == 0, result.stderr
    screenshots = sorted(screenshot_dir.glob("node_comments_variant_*.png"))
